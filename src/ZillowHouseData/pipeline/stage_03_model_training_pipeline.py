...
class DataModellingPipeline:
    ...
    def data_model(self):
        try:
            ...
            hyper_parameter_tuning = 0

            if hyper_parameter_tuning == 1:
                logger.info(">>>>>> Hyper-Parameter tuning Inititated <<<<<<\n\nx==========x")
                tuned_model = data_modeling.tune_model(X_train_scaled, y_train)
                logger.info(">>>>>> Hyper-Parameter tuning completed <<<<<<\n\nx==========x")
            else:
                logger.info(">>>>>> Loading pre-existing model <<<<<<\n\nx==========x")
